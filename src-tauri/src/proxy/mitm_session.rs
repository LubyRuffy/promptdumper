...
pub(crate) async fn run_mitm_session<'a, R, E>(
    ...
    client_base: hyper_util::client::legacy::Client<
    ...
    E: tauri::Emitter<R> + Clone + Send + Sync + 'static,
{
    ...
    let ctx = MitmRequestContext {
        app: app.clone(),
        llm_rules: llm_rules.clone(),
        client: client_base,
        peer,
        host: host.clone(),
        port,
    ...
    if negotiated_h2 {
        ...
        if let Ok(interval_ms) = std::env::var("PROXY_H2_PING_INTERVAL_MS")
            .and_then(|v| v.parse::<u64>().map_err(|_| std::env::VarError::NotPresent))
        {
            if interval_ms > 0 {
                h2_builder
                    .keep_alive_interval(std::time::Duration::from_millis(interval_ms.into()));
            }
        }
        if let Ok(timeout_ms) = std::env::var("PROXY_H2_PING_TIMEOUT_MS")
            .and_then(|v| v.parse::<u64>().map_err(|_| std::env::VarError::NotPresent))
        {
            if timeout_ms > 0 {
                h2_builder.keep_alive_timeout(std::time::Duration::from_millis(timeout_ms.into()));
            }
        }
        ...
        if h2_idle_secs == 0 {
            if let Err(err) = serve_fut.await {
                proxy_log!(
                    "[proxy][conn={}] http2 serve_connection error after {:?}: {}",
                    conn_id,
                    started.elapsed(),
                    err
                );
            ...
            let idle_task = {
                ...
                tokio::spawn(async move {
                    wait_idle(last, inflight, idle).await;
                })
            };
            tokio::select! {
                res = serve_fut => {
        ...
        let io = TokioIo::new(tls_stream);
        proxy_log!(
            "[proxy][conn={}] serving HTTP/1.1 for {}:{} (keep_alive=false)",
            conn_id,
            host,
            port
        );
        ...
        if let Err(e) = builder.serve_connection(io, service).await {
            proxy_log!(
                "[proxy][conn={}] http1 serve_connection error: {}",
                conn_id,
                e
            );
        }
    ...
    proxy_log!(
        "[proxy][conn={}] CONNECT session ended for {}:{}",
        conn_id,
        host,
        port
    );
